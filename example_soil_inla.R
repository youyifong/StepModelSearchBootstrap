...
# 
# library(chngpt)
# uninstall.packages("chngpt")
# uninstall.packages("kyotil")
# 
...
# data loading
setwd("C:/Users/kathj/OneDrive - USQ/Attachments/Projects/Youyi_thresholds/StepModelSearchBootstrap-main/data")
rdat <- read.csv("Ldry_withANOM_RAC_Feb2017.csv")
...
fit.gam.step.2=chngptm(formula.1=EVItrend~evap_anom+I(evap_anom^2)+I(evap_anom^3)+I(evap_anom^4)+I(evap_anom^5)+I(evap_anom^6)+I(evap_anom^7)+I(evap_anom^8)
    ...
    est.method="fastgrid2", var.type="bootstrap", save.boot=TRUE,verbose = 0, ci.bootstrap.size=510, ncpus=1)
...
###try model selection on gam (stricter collinearity ~0.5 cut-off)
...
formula = EVItrend ~ 
  ...
  f(inla.group(evap_anom,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  f(inla.group(midsoil_anom,n=80, method="quantile"),model="rw2", scale.model = TRUE) +
 # f(inla.group(topsoil_anom,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
 # f(inla.group(deepsoil_anom,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  
  # f(inla.group(s0_trend,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  # f(inla.group(sd_trend,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  # f(inla.group(ss_trend,n=30, method="quantile"),model="rw2", scale.model = TRUE) +

#  f(inla.group(e0_trend,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  
  f(inla.group(c3_c4ratio,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  f(inla.group(AHGF_FPC,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  f(inla.group(Dryag_prop,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
 # f(inla.group(Irriag_prop,n=30, method="quantile"),model="rw2", scale.model = TRUE) 
  
  # evap_anom*c3_c4ratio +
  # evap_anom*AHGF_FPC +
  # evap_anom*Dryag_prop +
  ...
  evap_anom*AHGF_FPC*c3_c4ratio*Dryag_prop
  # evap_anom*Dryag_prop*c3_c4ratio +
  # evap_anom*AHGF_FPC*Dryag_prop


 # f(inla.group(ag_ppt,n=30, method="quantile"),model="rw2", scale.model = TRUE) +
  # 
  #cent_ppt*cent_tmax +
  
  #tmax_g*rain_g + tmin_f*rain_f + #can have non-linear interaction - but have tried and AIC is much higher
  
 # f(loc_id.x, model="iid") + #f(loc_id.y, YR2, model="iid") #+ f(SITECOUNTRY, year, model="iid") 
  #f(YEAR, model="ar1")

##
#result = inla(formula, family="gaussian", data=rice_samp_year_sort,control.compute=list(dic=TRUE, waic=TRUE))

#summary(result) 

mod2  =  inla(formula, data = rdat, family = "gaussian", control.family = list(initial = 1),
...
#https://stats.stackexchange.com/questions/113940/in-r-and-inla-how-do-i-graph-results-of-rw2-model
#https://becarioprecario.bitbucket.io/inla-gitbook/ch-intro.html


#https://groups.google.com/g/r-inla-discussion-group/c/0dlvlVyiIRQ - INLA automatically fits in parallel !
...
mydata <- as.data.frame(mod2$summary.random$`inla.group(midsoil_anom, n = 80, method = "quantile")`)
#mydata <- as.data.frame(mod2$summary.random$`inla.group(c3_c4ratio, n = 30, method = "quantile")`)

#mod2$summary.fitted.values

#see here maybe http://www.leg.ufpr.br/~elias/cursos/montpellier2019/INLA_rinla.pdf 
#plot(mod2$summary.fitted.values$mean)
library(ggplot2)
ggplot() +   
  
  #geom_point(data=rice_samp_year_sort, aes(y=ag_tmin, x=ag_tmax)) + 
  
  geom_line(data=mydata, aes(ID, `0.5quant`)) +   
  geom_line(data=mydata,aes(ID, `0.025quant`), linetype="dashed") +   
  geom_line(data=mydata,aes(ID, `0.975quant`), linetype="dashed")  #+ ylim(0,2.5) + xlim(0,4)
